...
from nets import cnn, conv1d_block, sp_layer, asp_layer
...
outputs_std, outputs_mean = sp_layer(tcn_output)
# outputs = tf.concat([outputs_std, outputs_mean], axis=1)
# outputs_asp = asp_layer(cnn_output, training=bn_training)
...
with tf.Session(config=config) as sess:
    ...
    y_train_3, y_val_3 = np.concatenate([one_hot_y_data[:1059], one_hot_y_data[1535:]], axis=0), one_hot_y_data[1059:1535]
    x_train_4, x_val_4 = np.concatenate([x_data[:1535], x_data[2075:]], axis=0), x_data[1535:2075]
    y_train_4, y_val_4 = np.concatenate([one_hot_y_data[:1535], one_hot_y_data[2075:]], axis=0), one_hot_y_data[1535:2075]
    x_train_5, x_val_5 = np.concatenate([x_data[:2075], x_data[2596:]], axis=0), x_data[2075:2596]
    y_train_5, y_val_5 = np.concatenate([one_hot_y_data[:2075], one_hot_y_data[2596:]], axis=0), one_hot_y_data[2075:2596]
    x_train_6, x_val_6 = np.concatenate([x_data[:2596], x_data[3223:]], axis=0), x_data[2596:3223]
    y_train_6, y_val_6 = np.concatenate([one_hot_y_data[:2596], one_hot_y_data[3223:]], axis=0), one_hot_y_data[2596:3223]
    x_train_7, x_val_7 = np.concatenate([x_data[:3223], x_data[3745:]], axis=0), x_data[3223:3745]
    y_train_7, y_val_7 = np.concatenate([one_hot_y_data[:3223], one_hot_y_data[3745:]], axis=0), one_hot_y_data[3223:3745]
    x_train_8, x_val_8 = np.concatenate([x_data[:3745], x_data[4248:]], axis=0), x_data[3745:4248]
    y_train_8, y_val_8 = np.concatenate([one_hot_y_data[:3745], one_hot_y_data[4248:]], axis=0), one_hot_y_data[3745:4248]
    x_train_9, x_val_9 = np.concatenate([x_data[:4248], x_data[4826:]], axis=0), x_data[4248:4826]
    y_train_9, y_val_9 = np.concatenate([one_hot_y_data[:4248], one_hot_y_data[4826:]], axis=0), one_hot_y_data[4248:4826]
