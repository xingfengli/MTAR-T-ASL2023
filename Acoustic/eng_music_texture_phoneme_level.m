function [musicTexture_vec_phoneme,frame_Phoneme_dur]=eng_music_texture_phoneme_level(audioNames,fs,f0,loc,fMIDI_V,wordInfoOut_flag,dirType)
...
%[SFrm,EFrm,SegSy]=textread(['/Users/xingfengli/Documents/RESEARCH/MusicRep/iemocap/IEMOCAP/alignment/',au_name,'.wdseg'],'%s %s %s');
raw=importdata(['/Users/xingfengli/Documents/RESEARCH/MusicRep/iemocap/IEMOCAP/alignment/',au_name,'.phseg']);
if iscell(raw)
    strEnd_info=raw(2:end-1,1);
else
    strEnd_info=raw.textdata(2:end-1,1);
end
startTime=[];
endTime=[];
phStr=[];
for t=1:length(strEnd_info)
    idx=ismember(strEnd_info{t},' ');
    pos0=find(idx==0)';
    B = [0 find(diff(pos0')~=1) length(pos0')];
    %for i = 1:length(B)-1
    i=2;
    info_var2= pos0((B(i)+1):B(i+1));
    startTime=[startTime;str2num(strEnd_info{t}(min(info_var2):max(info_var2)))];
    i3=3;
    info_var3= pos0((B(i3)+1):B(i3+1));
    endTime=[endTime;str2num(strEnd_info{t}(min(info_var3):max(info_var3)))];
    i_phonem=length(B)-1;
    info_var4=pos0((B(i_phonem)+1):B(i_phonem+1));
    phStr=[phStr;{strEnd_info{t}(min(info_var4):max(info_var4))}]; 
end
...
% end
    ...
    start_end_syInfo=[ [(startTime/100)*fs (endTime/100)*fs];...
        [((endTime(end)+1)/100)*fs loc(end) ]];
    loc_boundary_info=start_end_syInfo;
    str_boundary_info=[phStr;{'SIL'}];
...
% 
%     musicTexture_vec_phoneme=[syllable_pos_sturcture_array...
%         frame_duration];%dur_frm_info syllable_duration];
%     %musicTexture_vec=syllable_pos_sturcture_array;
%     musicTexture_vec_phoneme=musicTexture_vec_phoneme(:,[1 2 5 6]);
    musicTexture_vec_phoneme=syllable_pos_sturcture_array;
    frame_Phoneme_dur=[ frame_duration syllable_duration ];
